...
!!! Info
    ...
    [Kubernetes' native `VolumeSnapshot` API](https://github.com/EnterpriseDB/cloud-native-postgres/issues/2081)
...
-   `pg_basebackup`: create a PostgreSQL cluster by cloning an existing one of
    ...
    useful if you want to migrate databases to EDB Postgres for Kubernetes, even
